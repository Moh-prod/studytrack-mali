...
import {
  Box, Typography, Card, CardContent, alpha, useTheme, Tooltip,
} from '@mui/material';
import {
...
function MonthlyReport({ entry, dailyEntries }) {
  ...
  const month = entry ? dayjs(entry.periodStart).month() + 1 : dayjs().month() + 1;
  ...
  const monthStats = useMemo(() => {
    ...
    const totalHabits = dailyEntries.reduce((s, r) => s + (r.habitsDone || 0), 0);
    const avgScore = Math.round(
      dailyEntries.reduce((s, r) => s + (r.productivityScore || 0), 0) / dailyEntries.length
    );
    const bestDay = dailyEntries.reduce(
      (best, r) => (r.productivityScore > (best?.productivityScore || 0) ? r : best),
      null
    );
    const activeDays = dailyEntries.filter((r) => r.tasksDone > 0 || r.habitsDone > 0).length;
    return { totalTasks, totalHabits, avgScore, bestDay, activeDays };
  }, [dailyEntries]);

  const badge = useMemo(() => getMonthlyBadge({
  ...
  const firstDayOfWeek = dayjs(`${year}-${String(month).padStart(2, '0')}-01`).day();
  ...
  return (
    <Box>
        ...
        >
          ...
          >
            <CardContent sx={{ py: 3 }}>
              ...
              >
                <Typography sx={{ fontSize: '3.5rem', display: 'block', mb: 1 }}>
                  {badge.emoji}
                </Typography>
              </motion.div>
              <Typography variant="h6" sx={{ fontWeight: 800, color: badge.color, mb: 0.5 }}>
                {badge.label}
              </Typography>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1.5, mb: 3 }}>
        {[
          { label: 'Tâches complétées', value: monthStats?.totalTasks || 0, icon: '✅', color: '#7C3AED' },
          { label: 'Jours actifs', value: `${monthStats?.activeDays || 0}/${heatmapData.length}`, icon: '📅', color: '#10B981' },
          { label: 'Habitudes cochées', value: monthStats?.totalHabits || 0, icon: '🔥', color: '#F59E0B' },
        ].map((stat, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
            <Card>
              <CardContent sx={{ textAlign: 'center', py: 2, '&:last-child': { pb: 2 } }}>
                <Typography sx={{ fontSize: '1.8rem', mb: 0.5 }}>{stat.icon}</Typography>
                <Typography sx={{ fontWeight: 800, fontSize: '1.3rem', color: stat.color }}>
                  {stat.value}
                </Typography>
        ...
        <CardContent>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 0.5, mb: 0.5 }}>
            {['L', 'M', 'M', 'J', 'V', 'S', 'D'].map((d, i) => (
              <Typography key={i} variant="caption" sx={{ textAlign: 'center', color: 'text.secondary', fontSize: '0.65rem', fontWeight: 700 }}>
                {d}
              </Typography>
            ))}
          </Box>
                ...
                >
                  <Box
                    sx={{
                      aspectRatio: '1',
                      borderRadius: 0.8,
                      backgroundColor: dayData.level > 0
                        ? HEATMAP_COLORS[dayData.level]
                        : alpha(theme.palette.mode === 'dark' ? '#fff' : '#000', 0.06),
                      border: dayData.level > 0
                        ? `1px solid ${alpha('#7C3AED', 0.2)}`
                        : `1px solid ${alpha(theme.palette.divider, 0.5)}`,
                      cursor: 'pointer',
                      boxShadow: dayData.level === 4
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mt: 2, justifyContent: 'flex-end' }}>
            <Typography variant="caption" sx={{ color: 'text.secondary', fontSize: '0.7rem' }}>Moins</Typography>
            {[0, 1, 2, 3, 4].map((level) => (
              <Box
                key={level}
                sx={{
            ...
            <Typography variant="caption" sx={{ color: 'text.secondary', fontSize: '0.7rem' }}>Plus</Typography>
          </Box>
        </CardContent>
      </Card>
    </Box>
