...
import { TrendingUpRounded, LightbulbRounded } from "@mui/icons-material";
    ...
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" sx={{ color: "text.secondary" }}>
          {entry.tasksTotal || 0} tâches · {entry.habitsTotal || 0} check
          habitudes · {entry.pomodoroSessions || 0} Pomodoros
        </Typography>
      </Box>
              ...
              {[
                {
                  ...
                  color: "#7C3AED",
                  progress: entry.completionRate || 0,
                },
                {
                  label: "Habitudes respectées",
                  value: `${entry.habitsDone || 0}/${entry.habitsTotal || 0}`,
                  color: "#10B981",
                  progress: entry.habitCompletionRate || 0,
                },
                {
                  label: "Sessions Pomodoro",
                  value: `${entry.pomodoroSessions || 0}`,
                  color: "#EF4444",
                  progress: Math.min(
                    ((entry.pomodoroSessions || 0) / 20) * 100,
                    100,
                  ),
                },
                {
                  label: "Meilleur jour",
                  value: entry.bestDayDate ? `${entry.bestDayScore}/100` : "—",
                  color: "#F59E0B",
                  progress: entry.bestDayScore || 0,
                },
              ].map((s, i) => (
                <motion.div
                  key={i}
          ...
          {dailyScores.length === 0 ? (
            ...
            >
              ...
              {Array.from({ length: 7 }, (_, i) => {
                const dayDate = dayjs(entry.periodStart)
                  .add(i, "day")
                  .format("YYYY-MM-DD");
                const dayData = dailyScores.find((d) => d.date === dayDate);
                const score = dayData?.score || 0;
                const barH =
                  score > 0 ? Math.max((score / maxScore) * 100, 8) : 4;
