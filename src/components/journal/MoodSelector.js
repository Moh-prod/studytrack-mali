...
function MoodSelector({ value, onChange, readOnly = false }) {
  ...
  return (
    <Box>
      <Typography
        variant="caption"
        sx={{ color: 'text.secondary', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.8, mb: 1.5, display: 'block' }}
      >
                ...
                <Box
                  ...
                  sx={{
                    ...
                    backgroundColor: isSelected
                      ...
                      : alpha(theme.palette.mode === 'dark' ? '#fff' : '#000', 0.04),
                    border: isSelected
      ...
      {value && (
        <motion.div initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}>
          ...
          >
            {MOODS.find((m) => m.key === value)?.emoji} {MOODS.find((m) => m.key === value)?.label}
          </Typography>
        </motion.div>
      )}
    </Box>
