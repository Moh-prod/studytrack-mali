...
function JournalPage({ user }) {
  ...
  const exportPDF = async () => {
    ...
    try {
      ...
      const opt = {
        margin: 10,
        filename: `Journal_${selectedEntry?.type}_${selectedEntry?.periodStart}.pdf`,
  ...
  const renderReport = () => {
    ...
    if (selectedEntry.type === "weekly") {
      return (
        <Box id="report-content" sx={{ p: 1 }}>
          <WeeklyReport key={selectedEntry.id} entry={selectedEntry} />
        </Box>
      );
    }
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <Box
                sx={{ display: "flex", alignItems: "center", gap: 0.8, mr: 2 }}
              >
                <AutoAwesomeRounded
