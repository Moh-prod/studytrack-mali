...
async function generateAlarmBlobUrl() {
  ...
  createTone(783.99, 0.30, 0.55, 0.18);   // G5

  // Brief pause, then resolution
  createTone(1046.50, 0.65, 0.7, 0.2);    // C6 — resolution, bright

  // Second phrase — repeat higher, more urgent
  createTone(659.25, 1.3, 0.35, 0.14);    // E5
  createTone(783.99, 1.45, 0.35, 0.16);   // G5
  createTone(1046.50, 1.60, 0.45, 0.18);  // C6
  createTone(1318.51, 1.80, 0.8, 0.15);   // E6 — gentle high resolution
...
export async function initAlarmSound() {
  ...
  generatePromise = (async () => {
    try {
      ...
      if (!OfflineCtx) {
        console.warn('OfflineAudioContext not supported, alarm will use fallback');
        return null;
      }
 ...
 * @param {number} loops - Number of times to play (default 3)
 * @returns {Promise<void>}
 */
export async function playAlarm(loops = 3) {
  // Try the pre-rendered audio first
  if (alarmAudio && alarmBlobUrl) {
    let played = 0;
 ...
 * (used when OfflineAudioContext is not available)
 */
function playFallbackAlarm() {
  try {
    ...
    const playBeep = (time, freq, dur = 0.3, vol = 0.18) => {
    ...
    // Play the alarm pattern twice
    for (let rep = 0; rep < 2; rep++) {
      const offset = rep * 2.2;
      playBeep(now + offset + 0.0, 523.25, 0.35);
      playBeep(now + offset + 0.15, 659.25, 0.35);
      playBeep(now + offset + 0.30, 783.99, 0.45);
      playBeep(now + offset + 0.60, 1046.50, 0.6, 0.2);
      playBeep(now + offset + 1.2, 659.25, 0.3);
      playBeep(now + offset + 1.35, 783.99, 0.3);
      playBeep(now + offset + 1.50, 1046.50, 0.4);
      playBeep(now + offset + 1.70, 1318.51, 0.5, 0.14);
    }
  } catch (e) {
