...
- Tâches prioritaires non faites : ${
    todayTasks
      .filter(
        (t) => !t.done && (t.priority === "urgent" || t.priority === "high"),
      )
      .map((t) => t.text)
      .join(", ") || "Aucune"
  }`;
...
/**
 * Envoie un message au coach IA avec l'historique de la conversation (Streaming).
 ...
 * @param {Function} onChunk - Callback(text) appelé à chaque morceau reçu
 * @returns {Promise<string>} Réponse complète du coach
 */
export async function streamChatWithCoach(history, message, context, onChunk) {
  ...
  try {
    ...
    console.warn("[AIService] Stream Chat error:", error);
    ...
    if (
      ...
      errorResponse =
        "Ta clé API Gemini semble incorrecte ou invalide. Assure-toi de l'avoir correctement générée sur Google AI Studio et configurée dans firebase.js ! 🔑";
    } else if (errMsg.includes("quota") || errMsg.includes("429")) {
      errorResponse =
        "J'ai atteint ma limite de requêtes pour le moment. Réessaie dans quelques minutes ! 😊";
    } else if (errMsg.includes("network") || errMsg.includes("fetch")) {
      errorResponse =
        "Il semble que tu n'aies pas de connexion internet. Reconnecte-toi et réessaie !";
    }
