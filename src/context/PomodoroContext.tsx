...
export function PomodoroProvider({
  ...
  const [workMin, setWorkMin] = useState(
    () => Number(localStorage.getItem("pomo_workMin")) || 25,
  );
  const [breakMin, setBreakMin] = useState(
    () => Number(localStorage.getItem("pomo_breakMin")) || 5,
  );
  const [longBreakMin, setLongBreakMin] = useState(
    () => Number(localStorage.getItem("pomo_longBreakMin")) || 15,
  );
  ...
  const [alarmSound, setAlarmSound] = useState(
    () => localStorage.getItem("pomo_alarmSound") || "default",
  );
  ...
  useEffect(() => {
    localStorage.setItem("pomo_alarmSound", alarmSound);
  }, [alarmSound]);
  ...
  const handleTimerComplete = useCallback(() => {
    ...
    // Note: playAlarm should be updated to accept the sound if possible
    playAlarm(3, alarmSound);
    ...
    if (isWorkRef.current) {
      ...
      if (user) {
        addDoc(collection(db, "pomodoro_sessions"), {
          uid: user.uid,
          duration: workMinRef.current,
          completedAt: new Date().toISOString(),
          taskId: linkedTaskId, // will be null if no task is linked
      ...
      if (linkedTaskId) {
        ...
        if (linkedTask) {
          const currentSpent = linkedTask.timeSpent || 0;
          updateTask(linkedTaskId, {
            timeSpent: currentSpent + workMinRef.current,
          });
        }
      }
      ...
      const body =
        "C'est reparti ! C'est l'heure de se concentrer et travailler.";
  ...
  useEffect(() => {
    ...
    const worker = new Worker("/pomodoroWorker.js");
  ...
  const totalTime = isWork
    ? workMin * 60
    : sessions > 0 && sessions % longBreakAfter === 0
      ? longBreakMin * 60
      : breakMin * 60;
  ...
  const skip = useCallback(() => {
    ...
    if (isWork) {
      ...
      setIsWork(false);
      const newTime =
        nextSessions % longBreakAfter === 0 ? longBreakMin * 60 : breakMin * 60;
      setTimeLeft(newTime);
      workerRef.current?.postMessage({ command: "reset", timeLeft: newTime });
    } else {
      setIsWork(true);
  ...
  // ─── Settings updaters ────────────────────────────────────────────
  const updateWorkMin = useCallback(
    (min) => {
      setWorkMin(min);
      if (!running && isWork) {
        setTimeLeft(min * 60);
  ...
  const updateLongBreakMin = useCallback(
    (min) => {
      setLongBreakMin(min);
      if (
        !running &&
